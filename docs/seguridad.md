...
| L5 | Autenticación | ✅ **Parcial**: el proxy ya no deja público un futuro `/api/health-xyz` (match exacto `/api/health` + prefijo `/api/health/`). Pendiente: reducir la verbosidad del payload público (expone `error.message`, presencia de env vars y tablas). | Reducir la verbosidad del payload de health. |
