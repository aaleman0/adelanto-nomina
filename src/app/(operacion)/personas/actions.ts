...
export async function resendSignedContractAction(formData: FormData) {
  ...
  if (!attempt?.easylex_contract_id) {
    redirect(`/personas/${employeeId}?action_status=not_found`);
  }
  ...
  revalidatePath(`/personas/${employeeId}`);
  redirect(
    `/personas/${employeeId}?action_status=${result.sent ? "contract_resent" : "contract_resend_failed"}`,
  );
}
