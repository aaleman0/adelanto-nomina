            ...
            <div className="mt-3 flex flex-col gap-2">
              {imports.map((imp) => (
                <button
                  ...
                  className={["flex w-full items-center justify-between rounded-lg border px-4 py-3 text-left", selectedImportId === imp.id ? "border-primary bg-primary-light" : "border-border bg-surface hover:bg-surface-muted"].join(" ")}
                >
                  <span className="text-sm font-medium text-text-primary">{imp.filename ?? "Importación sin nombre"}</span>
                  <span className="text-xs text-text-muted">{imp.total_rows ?? 0} empleados</span>
                </button>
              ))}
            </div>
          )}
