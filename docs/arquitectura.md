...
7. **Toda la auditoría pasa por el módulo compartido** (`src/lib/audit/`). Los helpers privados que quedaban en `request-contract.ts`, `mock-sign.ts` e `imports/apply.ts` ahora delegan en él; no hay inserts directos a `audit_events`/`integration_logs` fuera del módulo.
